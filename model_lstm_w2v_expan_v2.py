...
def model(X_train, Y_train, X_dev, Y_dev, learning_rate = 0.01, num_epochs = 10000,
    ...
    # tf Graph input
    X = tf.placeholder("float", [None, Tx, n_x], name = "X")
    ...
    weights = {
        ...
        'W_out' : tf.get_variable('W_out',[n_hidden,n_y], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    }
    biases = {
    ...
    num_correct = tf.reduce_sum(tf.cast(correct_pred, tf.float32), name = "model_accuracy")
