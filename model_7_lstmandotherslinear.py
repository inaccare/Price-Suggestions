...
def model(X_train, Y_train, trainCategories, trainBrands, trainConditions, X_dev, Y_dev, devCategories, devBrands, devConditions, learning_rate = 0.01, num_epochs = 10,
    ...
    weights = {
        ...
        'W_out' : tf.get_variable('W_out',[n_hidden + len_cat_vecs + len_brand_vecs + len_cond_vecs,n_y], initializer = tf.contrib.layers.xavier_initializer(seed = 1))
    }
    biases = {
        # 'out': tf.Variable(tf.random_normal([n_y]))
        'b_out' : tf.get_variable('b_out',[n_y], initializer = tf.zeros_initializer())
    }
        ...
        for step in range(1, num_epochs + 1):
            ...
            if step % display_step == 0 or step == 1:
                ...
                print("Epoch " + str(step) + ", Cost= " + \
                      "{:.6f}".format(epoch_cost/num_minibatches) + ", Training Mean percentage price difference (already in %)= " + \
                      "{:.5f}".format(float(100*tot_accuracy/m)))
...
def dynamicRNN(X, Cats, Brands, Conds, Tx, weights, biases, n_x, n_hidden):
    ...
    return tf.matmul(Z_out, weights['W_out']) + biases['b_out']
